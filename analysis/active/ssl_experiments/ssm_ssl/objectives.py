...
try:
    from ssl_core.patching import patched_lengths
except ModuleNotFoundError:  # pragma: no cover - repo-root unittest fallback
    from analysis.active.ssl_experiments.ssl_core.patching import patched_lengths
...
def expected_token_lengths_for_config(config: Any, input_lengths: torch.Tensor) -> torch.Tensor:
    ...
    if str(config.input_mode) == "causal_conv_stem":
        try:
            from ssl_core.patching import causal_conv_lengths
        except ModuleNotFoundError:  # pragma: no cover - repo-root unittest fallback
            from analysis.active.ssl_experiments.ssl_core.patching import causal_conv_lengths
