...
class SSLTrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mask_token_placement != "visible_only":
            raise ValueError(
                "mask_token_placement must be 'visible_only' for MAE training"
            )
        ...
        if self.decoder_backbone_direction not in {"causal", "bidirectional"}:
            raise ValueError("decoder_backbone_direction must be one of {'causal', 'bidirectional'}")
        if self.max_patches is not None and int(self.max_patches) <= 0:
            raise ValueError("max_patches must be positive when provided")
        if int(self.decoder_num_layers) <= 0:
    ...
    def checkpoint_config(self) -> dict[str, Any]:
        return {
            ...
            "max_patches": (None if self.max_patches is None else int(self.max_patches)),
            "decoder_hidden_size": (
                None if self.decoder_hidden_size is None else int(self.decoder_hidden_size)
            ),
            "decoder_s5_state_size": (
                None if self.decoder_s5_state_size is None else int(self.decoder_s5_state_size)
            ),
...
def _effective_max_patches(
    *,
    segment_bins: int,
    patch_size: int,
    patch_stride: int,
    explicit_max_patches: int | None,
) -> int:
    inferred = int(MAX_PATCH_COUNT(int(segment_bins), int(patch_size), int(patch_stride)))
    if explicit_max_patches is None:
        return inferred
    return max(int(explicit_max_patches), inferred)
    ...
    return {
        ...
        "max_patches": int(
            _effective_max_patches(
                ...
                patch_size=int(config.patch_size),
                patch_stride=int(config.patch_stride),
                explicit_max_patches=config.max_patches,
            )
        ),
        "decoder_hidden_size": (
            None if config.decoder_hidden_size is None else int(config.decoder_hidden_size)
        ),
        "decoder_s5_state_size": (
            None if config.decoder_s5_state_size is None else int(config.decoder_s5_state_size)
        ),
        "decoder_num_layers": int(config.decoder_num_layers),
        "decoder_dropout": float(config.decoder_dropout),
        "decoder_backbone_direction": str(config.decoder_backbone_direction),
        ...
        "use_normalization": bool(cache_context.use_normalization),
...
def recover_ssl_run_state_from_checkpoint(
    *,
    ...
    model = MaskedSSLModel(
        ...
        backbone_direction=str(recovered_config.get("backbone_direction", "bidirectional")),
        max_patches=int(recovered_config["max_patches"]),
        decoder_hidden_size=recovered_config.get("decoder_hidden_size"),
        decoder_s5_state_size=recovered_config.get("decoder_s5_state_size"),
        decoder_num_layers=int(recovered_config["decoder_num_layers"]),
        decoder_dropout=float(recovered_config["decoder_dropout"]),
        decoder_backbone_direction=str(recovered_config["decoder_backbone_direction"]),
    ).to(device)
...
def run_ssl_training(
    *,
    ...
    model = MaskedSSLModel(
        ...
        decoder_s5_state_size=config.decoder_s5_state_size,
        decoder_num_layers=int(config.decoder_num_layers),
        decoder_dropout=float(config.decoder_dropout),
        decoder_backbone_direction=str(config.decoder_backbone_direction),
    ).to(device)
    ...
    run_name = (
        f"colab_s5_mae_{config.objective_mode}_seg{int(config.segment_bins)}_"
