...
def recover_ssl_run_state_from_checkpoint(
    *,
    ...
    required_keys = [
        ...
        "dropout",
        "post_proj_norm",
        "reconstruction_head_mode",
        "reconstruction_head_type",
        "backbone_direction",
        "batch_size",
        "seed",
    ...
    cache_boundary_key_mode = str(cache_context.boundary_key_mode)
    if str(recovered_config["boundary_key_mode"]) != cache_boundary_key_mode:
        raise ValueError(
    ...
    model = MaskedSSLModel(
        input_dim=int(recovered_config["input_dim"]),
        ...
        post_proj_norm=str(recovered_config["post_proj_norm"]),
        source_session_keys=tuple(recovered_config.get("source_session_keys", ())),
        signal_spec=recovered_signal_spec,
        reconstruction_head_mode=str(recovered_config["reconstruction_head_mode"]),
        reconstruction_head_type=str(recovered_config["reconstruction_head_type"]),
        backbone_direction=str(recovered_config["backbone_direction"]),
    ).to(device)
