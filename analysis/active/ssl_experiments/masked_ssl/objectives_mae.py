...
from .model_mae import MaskedSSLModel
...
def summarize_metrics(metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    if "requested_mask_ratio" in metrics:
        summary["requested_mask_ratio"] = float(metrics["requested_mask_ratio"])
...
def build_masked_batch(
    ...
    *,
    ...
    # MAE supervises all masked patches; no causal span-start workaround is needed.
    score_full_patch_spans = True
    ...
    if mask_unit == "patch":
        ...
        # Ensure at least one visible patch per sample for MAE encoder input.
        for sample_idx, token_length in enumerate(token_lengths.tolist()):
            if token_length <= 0:
                continue
            if bool(token_mask[sample_idx, :token_length].all().item()):
                token_mask[sample_idx, token_length - 1] = False
