...
This note is the current concise results record for the POSSM reconstruction
and Brain2Text24 phoneme-decoding experiments. Implementation and recovery
details remain in `EXPERIMENT_NOTES.md`; the longer historical record is
archived at `docs/notes/archive/possm_reproduction_results.md`.

## Brief historical summary

The archived experiments established that POSSM reconstruction pretraining can
materially improve subsequent CTC optimization, but also exposed substantial
sensitivity to the signal and Stage-2 temporal interface.

- Early TX-only pretrained POSSM-GRU runs reached approximately `PER=0.58`,
  compared with approximately `0.71` from random initialization. The random
  model was much more blank-dominant and under-emissive.
- A lower-dropout 3,000-step TX run reached `PER=0.523`; continuing it to 4,000
  steps produced only a small improvement to approximately `0.517` while the
  train/validation CTC gap widened.
- The later 12,000-step area-6v TX run reached `PER=0.467`, whereas its matched
  random-init comparison remained around `0.707-0.728`. This is the clearest
  archived evidence that Stage-1 initialization helps the POSSM-GRU path.
- Replacing the GRU interface with causal S5, including a pre-decoder-patching
  variant, did not improve POSSM transfer; the latter plateaued around
  `PER=0.56`.
- S5 itself was not the limiting factor. Supervised Willett-style S5 reached
  `PER=0.336` with TX and `0.256` with TX+SBP when supplied with the supervised
  normalization, augmentation, and pre-decoder temporal-patching recipe.

The historical progression therefore supports reconstruction pretraining, but
does not yet isolate whether the strongest current gain comes from SBP, the
additional Brain2Text25 data, or both.
