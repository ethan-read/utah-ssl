...
```bash
RECOMPUTE_SPLIT_STATS=0 scripts/runpod/launch_willett_s5_tx_sbp.sh
```
...
```bash
AUTO_STOP_POD_ON_EXIT=0 scripts/runpod/launch_willett_s5_tx_sbp.sh
```
...
python -m pip install -r scripts/runpod/requirements-willett.txt
python -m unittest analysis.active.ssl_experiments.willett_reconstruction.tests.test_willett_reconstruction -q
```
...
```bash
chmod +x scripts/runpod/launch_willett_s5_tx_sbp.sh
...
MAX_STEPS=60000 \
SEED=7 \
scripts/runpod/launch_willett_s5_tx_sbp.sh
```
...
nohup env \
  ...
  MAX_STEPS=60000 \
  SEED=7 \
  scripts/runpod/launch_willett_s5_tx_sbp_cv.sh \
