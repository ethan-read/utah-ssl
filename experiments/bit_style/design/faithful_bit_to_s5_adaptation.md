...
Repository policy differs from literal BIT fidelity at the downstream input:
the default Brain-to-Text signal is 128-channel area-6v SBP from the validated
clipped-FP16 cache. TX+SBP should therefore be treated as an explicit
paper-fidelity comparison. Both variants keep the area-6v restriction, and any
SBP component comes from the clipped-FP16 cache.
...
- same neural inputs for the explicit paper-fidelity comparison: area-6v
  `TX + SBP`, with SBP loaded from the clipped-FP16 cache
...
For the explicit paper-fidelity comparison, use the same neural feature policy
as BIT:
...
Run the repository-default SBP-only downstream condition alongside it. This
separates fidelity to BIT's input choice from the question of which signal is
most reliable for the Brain-to-Text datasets.
