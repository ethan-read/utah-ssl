# POSSM-style experiment handoff notes

This is the compact handoff for the current paper-derived POSSM-style
reconstruction and transfer experiments. No released POSSM code was available,
so the implementation reflects inferences from the paper rather than an
official or verified reproduction. The implementation lives in this folder;
reusable cache and normalization artifacts live under the Drive data root, not
in experiment output directories.
...
- `notebooks/pooled_brain2text24_brain2text25_pretraining.ipynb`: current Colab workflow.
  ...
  expected.
- `notebooks/pooled_cache_preparation.ipynb`: one-time workflow that
...
- `model.py`: POSSM encoder and phoneme model architecture.
- `training.py`: Stage-1 training, checkpoints, recovery, and
  progress serialization.
- `phoneme_finetune.py`: Stage-2 data loading, fine-tuning,
  checkpoint recovery, CTC loss, PER, and timing logs.
- `../../../docs/data/signal_and_dataset_contracts.md`: the current modular interface for choosing
  datasets and neural signals across cache inspection, analysis, and training.
- `../../../utah_ssl/cache.py`: shared cache context and
...
- `../../../docs/data/cache_and_stats_inventory.md`: canonical cache roots and stats
  inventory.
- `../results/README.md`: canonical completed reconstruction and transfer
  results, including the earlier area-6v TX comparison.
- `../../../docs/research_status.md`: compact cross-experiment interpretation
...
The current shared signal, dataset, and checkpoint guarantees are documented in
[`../../../docs/data/signal_and_dataset_contracts.md`](../../../docs/data/signal_and_dataset_contracts.md).
...
Fresh pooled runs use these Brain2Text25-only clipped-FP16 SBP roots:

- raw:
  `utah_ssl/data/cache_v1_possm_b2t25_area6v_sbpclip12500_fp16_raw_v1`
- smoothed:
  `utah_ssl/data/cache_v1_possm_b2t25_area6v_sbpclip12500_fp16_smoothed_v1`

Brain2Text24 uses the corresponding shared roots:

- raw: `utah_ssl/data/cache_v1_sbpclip12500_fp16_raw`
- smoothed: `utah_ssl/data/cache_v1_sbpclip12500_fp16_smoothed`
...
Brain2Text24 shard size. TX, labels, and offsets retain their source values;
SBP is clipped using the validated threshold and stored as FP16 to reduce
storage and I/O without a measurable decoding-accuracy loss. Building the
smoothed destination from the existing smoothed source is deliberate:
smoothing is not regenerated after examples have been assigned to new shard
boundaries.
...
Use `drive_direct` first; switch the pooled-pretraining notebook's
`STAGE1_CACHE_MODE` to `copy_to_local` only if the cache-preparation notebook's
warm benchmark remains slow.

Each run still records an explicit `DatasetPlan` and `SignalSpec`, but the
POSSM-style Brain-to-Text default is `sbp_only` over 128 area-6v channels from
the clipped-FP16 cache. `tx_only` and `tx_sbp` are explicit modality
comparisons, not alternative defaults. Cache loading, statistics, raw access,
manifold/probe construction, and model training consume the same objects.
Current checkpoints and stats artifacts must contain both contracts;
...
stats namespace:
`stats/session_feature_stats/sbpclip12500_fp16_smoothed_mixed_<source-signature>/`.
...
The active pooled workflow should expose only two required normalization
artifacts: pooled clipped-FP16 pre-smoothed SBP session stats for Stage 1 and
clipped-FP16 raw Brain2Text24 `competition_train` SBP global stats for Stage 2. Add the
...
reproduction inputs, not current defaults. Use `utah_ssl.stats` from Python and
