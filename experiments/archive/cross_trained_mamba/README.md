...
> **Archived.** This specific Tether-Evo-inspired Mamba transfer design was
> deprioritized after a poor preliminary run and a shift in research focus. The
> code, tests, Modal launcher, design record, and output conventions are retained here. Run tests
> from the repository root with
> `python -m unittest discover -s experiments/archive/cross_trained_mamba/tests`;
> artifacts use the output roots documented below.

## What was actually examined

Only a limited BIT-style Mamba run was attempted, and it performed very poorly.
The branch was not studied in depth: there was no serious hyperparameter sweep,
multi-seed comparison, scaling study, or controlled investigation of which
architectural choice caused the failure. No completed cross-dataset result was
established.

The remainder of this document is primarily the proposed experiment design and
implementation record. It should not be read as evidence that the complete
Tether-Evo-style study was executed, or as evidence that Mamba is generally
unsuitable for neural decoding.

Current data policy supersedes the historical cache discussion below:
Brain-to-Text 2024 and 2025 use 128-channel area-6v views, and any SBP component
comes from the validated clipped-FP16 cache. TX+SBP is an explicit comparison,
not the default Brain-to-Text signal.
...
- the existing archived Mamba future-prediction runtime work, especially
...
- `experiments/archive/mamba_future_prediction/design/design_decisions.md`
- `experiments/archive/mamba_future_prediction/model.py`
- `experiments/bit_style/model.py`
- `experiments/archive/mamba_future_prediction/launchers/modal/run_future_prediction.py`

Reusable ideas:

- Use the BIT-style Mamba wrapper as the implementation anchor:
...
- `experiments/supervised_baselines/results/willett_gru_replication.md`
- `experiments/supervised_baselines/launchers/runpod/README.md`
- `docs/research_status.md`
- `docs/data/cache_and_stats_inventory.md`
- `docs/data/canonical_drive_cache_spec.md`
- `experiments/supervised_baselines/model.py`
- `experiments/supervised_baselines/train.py`
...
- `experiments/archive/cross_trained_mamba/launchers/modal/run_cross_trained_mamba.py`

The launcher should copy:

- `experiments/archive/cross_trained_mamba`;
- `experiments/supervised_baselines`;
- `utah_ssl`;
- `experiments/bit_style`;
- any stats recomputation helpers it imports.

Before training, the launcher must run the same Mamba fast-kernel diagnostic
used by `experiments/archive/mamba_future_prediction/launchers/modal/run_future_prediction.py` and fail closed if the
...
6. Implement hierarchical Mamba stages and verify output shapes:
   ...
   - patch token lengths match `utah_ssl.patching`;
