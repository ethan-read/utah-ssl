...
> **Archived.** This automated S5/SSL benchmark scaffold and its local smoke
> harness were deprioritized when the research direction shifted away from
> broad architecture search. Code, runtime profiles, commands, tests-by-smoke,
> and output conventions are retained. The last-known environments are the
> `local_debug`, `single_gpu`, `colab_cuda`, and `cluster_cuda` profiles below;
> generated artifacts use the documented output root.
